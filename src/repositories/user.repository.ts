...
export const makeUserRepository = (db: NodePgDatabase) => {
	...
	const findById = async (id: string) => {
		console.log(`Search for user ID: ${id}`);
	...
	const updateUser = async (id: string, updates: Partial<Omit<Database.User, "id" | "createdAt">>) => {
	 ...
	 */
	const findByIds = async (ids: string[]): Promise<Database.User[]> => {
		if (!ids.length) return [];
